...
plt.plot(freqs, bdust(freqs,z=0.36315244))
#plt.yticks(rotation=70)
plt.ticklabel_format(axis='y',style='sci',scilimits=(0, 4))
plt.ylabel(r'$b(\nu)$')
plt.xlabel(r'$\nu$ (GHz)')
plt.savefig('bdust.pdf')
plt.close()
Tcmb=2.7255

plt.plot(freqs,10^{-17}*Tcmb*gfuncrel(freqs*1e9,Te=0,),label='$a_i\timesY$ (Teff=0 keV_')
plt.plot(freqs,10^{-17}*Tcmb*gfuncrel(freqs*1e9,Te=58022625.03083,),label='$a_i\timesY$(Teff=5 keV)') 
plt.plot(freqs,10^{-17}*Tcmb*gfuncrel(freqs*1e9,Te=116045250.0617,),label='$a_i\timesY$(Teff=10 keV)') 
plt.plot(freqs,10^{-17}*Tcmb*gfuncrel(freqs*1e9,Te=174067875.0925,),label='$a_i\timesY$(Teff=15 keV)')
lt.plot(freqs, 10^{-18}*bdust(freqs,z=0.36315244),label='b_i D_c')
plt.ylabel(r'$g(\nu)$')
plt.xlabel(r'$\nu$ (GHz)')
plt.legend()
plt.savefig('gnu&bdust.pdf')
plt.close()
