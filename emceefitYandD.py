...
def lnlike(param,nu,S,yerr): 
    ...
    Y,D=param
    nu=np.asarray(nu)
    model=yflux(nu,Y)+dflux(nu,D)#+Y * gfuncrel(nu*1e9,Teff)
...
Y_mcmc, D_mcmc= map(lambda v: (v[1], v[4]-v[3],v[3]-v[2],v[2]-v[1], v[1]-v[0]),
                             zip(*np.percentile(samples, [5,32,50,68,95],
                                                axis=0)))
print(Y_mcmc, D_mcmc)#,Teff_mcmc)
print(np.mean(Y_samples),np.mean(D_samples),np.mean(Teff_samples))
# print(Y_mcmc[0])
# print(D_mcmc[0])
# print(Teff_mcmc[0])
...
#                       #truths=[Y, D])
# fig.savefig(io.dout_dir+"CornerTeffplot.pdf")
...
# pl.legend()
# pl.done(io.dout_dir+"apfluxes_Tefffitlog.pdf")
...
# pl.add_err(x,np.abs(apmeans),yerr=aperrs,marker="o",ls="none")
# pl.legend()
# pl.done(io.dout_dir+"apfluxes_Tefffitlinear.pdf")
